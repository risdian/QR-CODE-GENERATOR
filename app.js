...
document.querySelectorAll(".tab").forEach(btn => {
    btn.addEventListener("click", () => {
        const tab = btn.dataset.tab.toLowerCase().replace("location", "map").replace("vcard", "vcard").replace("url", "url").replace("text", "text");
        currentTab = tab;
...
function renderQR() {
    ...
    // High-res canvas for export, displayed canvas shown at preview size
    ...
    function finishAndShow() {
        ...
        // Show canvas scaled to preview size
    ...
    if (logoDataUrl) {
        const img = new Image();
        img.onload = () => {
            // Logo fills ~22% of QR width, centered
...
document.getElementById("qrTheme").addEventListener("change", () => {
...
function copyQR() {
    ...
    currentCanvas.toBlob(blob => {
        try {
            ...
            ]).then(() => showToast()).catch(() => fallbackCopy());
        } catch {
            fallbackCopy();
        }
    }, "image/png");
...
function fallbackCopy() {
    // Fallback: download instead if clipboard API unavailable
    downloadQR();
}
